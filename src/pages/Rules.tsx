...
export function RulesPage() {
  ...
  const toggleAuthenticator = (a: string) => {
    if (!rule) return;
    applyChange("update", `oathkeeper: ${rule.id} · authenticator ${a}`, () => {
      setState(s => ({
        ...s, accessRules: s.accessRules.map(r => {
          if (r.id !== rule.id) return r;
          const has = r.authenticators.includes(a);
          return { ...r, authenticators: has ? r.authenticators.filter(x => x !== a) : [...r.authenticators, a] };
        })
      }));
    });
  };
  ...
  const setAuthorizer = (val: string) => {
    if (!rule) return;
    applyChange("update", `oathkeeper: ${rule.id} · authorizer=${val}`, () => {
      setState(s => ({ ...s, accessRules: s.accessRules.map(r => r.id === rule.id ? { ...r, authorizer: val } : r) }));
    });
  };
            ...
            <div className="panel-body col" style={{ gap: 16 }}>
              <div>
                ...
                <div className="panel" style={{ padding: 10 }}>
                  <div className="mono small"><span className="muted">url: </span>{rule.match.url}</div>
                  <div className="mono small mt-4"><span className="muted">methods: </span>{rule.match.methods.join(", ")}</div>
                </div>
              </div>
              <div>
              ...
              <div>
                <label className="input-label">Upstream</label>
                <div className="panel" style={{ padding: 10 }}><div className="mono small">{rule.upstream || "— none —"}</div></div>
              </div>
